document.addEventListener('DOMContentLoaded', function() {
  ...
  function selectAllCards() {
    uploadedCards.forEach(card => {
      ...
      const cardElement = document.querySelector(`.card-preview[data-card-id="${card.id}"]`);
      if (cardElement) {
        cardElement.classList.add('selected');
      }
    });
  ...
  function initFromLocalStorage() {
    ...
    if (formState.selectedCards && formState.selectedCards.length > 0) {
      ...
      selectedCards.forEach(cardId => {
        const cardElement = document.querySelector(`.card-preview[data-card-id="${cardId}"]`);
        if (cardElement) {
          cardElement.classList.add('selected');
        }
      });
  ...
  function clearForm() {
    ...
    // Clear card previews
    cardPreviewContainer.innerHTML = '<p class="no-cards">No cards uploaded yet. Upload images to see previews.</p>';
